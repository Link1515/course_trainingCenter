...
app.get('/', async (req, res) => {
  try {
    ...
    if (req.query.agelte && !isNaN(parseInt(req.query.agelte))) {
      query.$and.push({ age: { $lte: req.query.agelte } })
    }
    if (req.query.agegte && !isNaN(parseInt(req.query.agegte))) {
      query.$and.push({ age: { $gte: req.query.agegte } })
    }

    // $and 查詢時不能空白，所以如果沒有查詢時就刪除
