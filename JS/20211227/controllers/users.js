  ...
  try {
    ...
    if (user === null) {
      ...
      const token = jwt.sign(
        { _id: user._id.toString() }, process.env.SECRET, { expiresIn: '1 s' }
      )
      user.jwt.push(token)
      await user.save()
      res.status(200).send({ success: true, messgae: '', result: { token } })
    }
  } catch (error) {
    console.log(error)
    res.status(500).send({ success: false, message: '伺服器錯誤' })
...
export const logout = async (req, res) => {
  try {
    ...
    res.status(500).send({ success: false, message: '未登入' })
  }
}
...
export const extend = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { _id: req.user._id.toString() }, process.env.SECRET, { expiresIn: '7 days' }
    )
